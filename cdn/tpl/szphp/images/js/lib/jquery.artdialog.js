...
var ctrl_id = "c";
var func_id = "f";
var webroot = "http://www.szyttv.com/";
var apifile = "api.php";
var lang= new Array();

function get_url(ctrl,func,ext){var url = "http://www.szyttv.com/index.php?c="+ctrl;if(func){url+="&f="+func;};if(ext){url+="&"+ext};return url;}
function get_plugin_url(id,func,ext){var url = "http://www.szyttv.com/index.php?c=plugin&f=index&id="+id+"&exec="+func;if(ext){url+="&"+ext};url+="&_noCache="+Math.random();return url;};
function api_url(ctrl,func,ext){var url = "http://www.szyttv.com/api.php?c="+ctrl;if(func){url+="&f="+func;};if(ext){url+="&"+ext};url+="&_noCache="+Math.random();return url;};
function api_plugin_url(id,func,ext){var url = "http://www.szyttv.com/api.php?c=plugin&f=index&id="+id+"&exec="+func;if(ext){url+="&"+ext};url+="&_noCache="+Math.random();return url;};
...
/*下面这段是微信二维码弹出++++++++++20170323添加*/
function showLayer(id){var layer=$('#'+id),layerwrap=layer.find('.hw-layer-wrap');layer.fadeIn();layerwrap.css({'margin-top':-layerwrap.outerHeight()/2})}function hideLayer(){$('.hw-overlay').fadeOut()}$('.hwLayer-ok,.hwLayer-cancel,.hwLayer-close').on('click',function(){hideLayer()});$('.show-layer').on('click',function(){var layerid=$(this).data('show-layer');showLayer(layerid)});$('.hw-overlay').on('click',function(event){if(event.target==this){hideLayer()}});$(document).keyup(function(event){if(event.keyCode==27){hideLayer()}})
